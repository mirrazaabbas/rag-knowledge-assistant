...
Render Postgres supports the `pgvector` extension on PostgreSQL 13+, including PostgreSQL 16. The application runs `CREATE EXTENSION IF NOT EXISTS vector` during schema initialization.

### Free-tier accuracy boundary

The Blueprint intentionally uses Render's free service and free Postgres plans so it can serve as a portfolio demo without assuming paid infrastructure. Render documents free Postgres as temporary demo infrastructure with a 30-day database lifetime and no backups. Do not describe the free deployment as a production SLA. For a durable deployment, upgrade the database plan and configure backups/retention appropriate to the use case.
