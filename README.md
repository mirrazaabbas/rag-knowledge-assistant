...
A source-grounded RAG portfolio project with FastAPI, a transparent TF-IDF baseline, PostgreSQL/pgvector semantic retrieval, cited answer generation, OpenTelemetry/OTLP tracing, Docker, CI, benchmark evidence, and deployment infrastructure.
...
GitHub Actions additionally starts a real pgvector service, executes integration tests, runs the retrieval benchmark, uploads the raw benchmark JSON, builds the production Docker image, smoke-tests its health/readiness endpoints, and verifies OTLP tracing against Jaeger.
...
Python · FastAPI · Pydantic · RAG · Information Retrieval · Embeddings · LLM Integration · PostgreSQL · pgvector · Vector Search · HNSW · REST APIs · Source Grounding · Docker · Docker Compose · OpenTelemetry · OTLP · Jaeger · Testing · CI/CD · Benchmarking · Cloud Deployment Configuration

## Scope and accuracy

The local baseline, PostgreSQL/pgvector integration path, integration tests, CI benchmark, deployment configuration, production container verification, and local/CI OTLP tracing path are implemented and verified where stated. The repository does **not** claim a public cloud URL, live external-provider verification, or a durable production tracing backend until those are actually performed.
